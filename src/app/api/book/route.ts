...
import { getVehicleClass } from "@/config/vehicle";
import type {
  ApiResult,
  BookingItem,
  BookingPhoto,
...
export async function POST(request: Request) {
  ...
  /**
   * Класс машины проверяем по справочнику, а не берём из запроса:
   * иначе достаточно прислать свой множитель, чтобы «насчитать» себе
   * скидку на ремонт подвески внедорожника.
   */
  const vehicleClass = getVehicleClass(body.vehicle?.classId);
  if (!vehicleClass) fields.vehicle = "Выберите класс автомобиля";
  if (!body.vehicle?.model || body.vehicle.model.trim().length < 2) {
    fields.vehicle = "Укажите марку и модель";
  }

  const summary = summarize(rawItems, vehicleClass?.id ?? null);
  ...
  const record = await bookingRepository.create({
    lines: toLines(rawItems, vehicleClass!.id),
    vehicle: {
      classId: vehicleClass!.id,
      model: body.vehicle!.model.trim().slice(0, 80),
      plate: (body.vehicle?.plate ?? "").trim().slice(0, 16),
    },
    vehicleClassLabel: vehicleClass!.label,
...
export async function GET(request: Request) {
  ...
  const classId = searchParams.get("vehicleClass");
  ...
  const summary = summarize(items, classId);
