...
function SettingsTab({ tab }: { tab: Tab }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "masters" && (
        <Card title="Механики" hint="Клиент увидит только тех, кто делает все выбранные работы">
    ...
    <div className="space-y-2">
      {items.map((b) => (
        <div key={b.id} className="rounded-control border border-line p-4">
          ...
          {/* Машина сразу под контактами: механику это нужнее фамилии */}
          <p className="mt-1 text-sm">
            {b.vehicle?.model}
            {b.vehicle?.plate && <span className="text-muted"> · {b.vehicle.plate}</span>}
            {b.vehicleClassLabel && (
              <span className="text-muted"> · {b.vehicleClassLabel}</span>
            )}
          </p>
