...
const emptyDraft: BookingDraft = {
  items: [],
  vehicle: { classId: null, model: "", plate: "" },
  masterId: null,
  date: null,
  time: null,
  name: "",
...
interface BookingContextValue {
  ...
  /** класс машины меняет длительность работ, поэтому сбрасывает выбранное время */
  setVehicleClass: (classId: string) => void;
  step: BookingStep;
...
export function BookingProvider({ children }: { children: React.ReactNode }) {
  ...
  const setVehicleClass = useCallback((classId: string) => {
    setDraft((d) => ({ ...d, vehicle: { ...d.vehicle, classId }, time: null }));
    setFieldErrors({});
  }, []);
  ...
  const submit = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      if (res.fields?.items || res.fields?.vehicle) setStep(1);
  ...
  const value = useMemo(
    () => ({
      draft, patch, toggle, setVehicleClass, step, goTo: setStep, isOpen, open, close,
      status, error, fieldErrors, result, submit, reset,
    }),
    [draft, patch, toggle, setVehicleClass, step, isOpen, open, close, status, error, fieldErrors, result, submit, reset],
  );
