...
export function StepWhen() {
  ...
  const summary = useMemo(() => summarize(draft.items, draft.vehicle.classId), [draft.items, draft.vehicle.classId]);

  // Механика, которые делают ВСЕ выбранные процедуры.
  ...
  if (summary.items.length === 0) {
    return <p className="text-muted">Сначала выберите хотя бы одну работу.</p>;
  }
  ...
  if (exceedsWorkday(data, summary.durationMin)) {
    return (
      <p className="rounded-control bg-surface px-4 py-5 text-sm text-muted">
        ...
        Вернитесь на шаг назад и уберите одну работу.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {eligible.length > 0 && (
        <div>
          <p className="mb-3 text-sm text-muted">Механик</p>
          ...
          <p className="mt-2 text-xs text-muted">
            {draft.masterId
              ...
              : "Свободное время любого свободного механика"}
          </p>
        </div>
      )}
      ...
      <div>
        <div className="mb-3 flex items-baseline justify-between gap-3">
          ...
          <p className="text-xs text-muted">
            Работы займут {formatDuration(summary.durationMin)}
          </p>
        </div>
        ...
        {!draft.date ? (
          ...
          <p className="rounded-control bg-surface px-4 py-5 text-sm text-muted">
            ...
            подряд. Посмотрите соседние даты или уберите одну работу.
          </p>
        ) : (
