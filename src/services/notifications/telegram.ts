...
export function buildMessage(b: BookingRecord) {
  // Время окончания считаем сами: мастеру важнее знать, до скольки он занят,
  // чем складывать длительности в уме.
  const [h, m] = b.time.split(":").map(Number);
  const endMin = h * 60 + m + b.totalDurationMin;
  const end = `${String(Math.floor(endMin / 60)).padStart(2, "0")}:${String(endMin % 60).padStart(2, "0")}`;

  const services = b.lines
    .map((l) => `  • ${esc(l.serviceTitle)} — ${esc(l.variantLabel)}, ${formatDuration(l.durationMin)}, ${formatPrice(l.price, b.currency)}`)
    .join("\n");
  ...
  const lines = [
    ...
    "",
    `<b>Машина:</b> ${esc(b.vehicle.model)}${b.vehicle.plate ? ` · ${esc(b.vehicle.plate)}` : ""}`,
    `<b>Класс:</b> ${esc(b.vehicleClassLabel)}`,
    "",
    `<b>Работы (${b.lines.length}):</b>`,
    services,
    "",
    `<b>Расчёт:</b> ${formatDuration(b.totalDurationMin)} · ≈ ${formatPrice(b.totalPrice, b.currency)} <i>(без запчастей)</i>`,
    `<b>Механик:</b> ${esc(b.masterName)}`,
    `<b>Дата:</b> ${formatDateLong(b.date)}`,
    `<b>Время:</b> ${b.time} — ${end}`,
  ];
